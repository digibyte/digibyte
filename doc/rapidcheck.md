# RapidCheck property-based testing for DigiByte Core

## Concept

Property-based testing is experimentally being added to DigiByte Core with
...
If RapidCheck is installed, DigiByte Core will automatically run the
...
3. Configure DigiByte Core with RapidCheck.

    `cd` to the directory of your local digibyte repository and run
...
4. Build DigiByte Core with RapidCheck.
    ...
    Making all in src
    [...]
    CXX      test/gen/test_digibyte-crypto_gen.o
    CXX      test/test_digibyte-key_properties.o
    ```
...
That's it! You are now running property-based tests in DigiByte Core.
