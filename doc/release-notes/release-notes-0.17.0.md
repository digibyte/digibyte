DigiByte Core version 0.17.0 is now available from:
  ...
  <https://digibytecore.org/bin/digibyte-core-0.17.0/>
...
Previously, wallets could only be loaded or created at startup, by specifying `-wallet` parameters on the command line or in the digibyte.conf file. It is now possible to load, create and unload wallets dynamically at runtime:
...
be used in V0.17 by starting digibyted with the '-deprecatedrpc=accounts'
...
[BIP 174 PSBT](https://github.com/digibyte/bips/blob/master/bip-0174.mediawiki) is an interchange format for DigiByte transactions that are not fully signed
yet, together with relevant metadata to help entities work towards signing it.
It is intended to simplify workflows where multiple parties need to cooperate to
produce a transaction. Examples include hardware wallets, multisig setups, and
[CoinJoin](https://digibytetalk.org/?topic=279249) transactions.
...
- `signrawtransaction` is deprecated and will be fully removed in v0.18. To use
  `signrawtransaction` in v0.17, restart digibyted with
...
- #11962 `0a01843` add seed.digibyte.sprovoost.nl to DNS seeds (Sjors)
...
- #12723 `310dc61` Qt5: Warning users about invalid-BIP21 URI digibyte:// (krab)
...
- #13486 `66e1a08` Move rpc/util.cpp from libdigibyte-util to libdigibyte-server (ken2812221)
...
- #13604 `75bea05` Add depends 32-bit arm support for digibyte-qt (TheCharlatan)
...
- #13696 `2ab7208` Add aarch64 qt depends support for cross compiling digibyte-qt (TheCharlatan)
...
- #12443 `07090c5` Move common args to digibyte.conf (MarcoFalke)
...
- #12882 `d96bdd7` Make `test_digibyte` pass under ThreadSanitzer (clang). Fix lock-order-inversion (potential deadlock) (practicalswift)
...
- #13687 `9d26b69` travis: Check that ~/.digibyte is never created (MarcoFalke)
...
- #13811 `660abc1` travis: Run `bench_digibyte` once (MarcoFalke)
...
- #12102 `7fb8fb4` Apply hardening measures in digibyted systemd service file (Flowdalic)
- #12652 `55f490a` digibyte-cli: Provide a better error message when digibyted is not running (practicalswift)
...
- #12769 `35eb9d6` Add systemd service to digibyted in debian package (ghost)
...
- #13374 `56f6936` utils and libraries: checking for digibyte address in translations (kaplanmaxe)
...
- #13603 `453ae5e` digibyte-tx: Stricter check for valid integers (domob1812)
...
- #13725 `07ce278` Fix digibyte-cli --version (Empact)
...
- #12877 `2b54155` Use digibyted in Tor documentation (knoxcard)
...
- #13346 `60f0358` update digibyte-dot-org links in release-process.md (fanquake)
...
- #13742 `d2186b3` Adjust digibytecore.org links (MarcoFalke)
...
- #13905 `73a09b4` fixed digibyte-cli -help output for help2man (hebasto)
