DigiByte Core version 0.18.0 is now available from:
  ...
  <https://digibyte.org/bin/digibyte-core-0.18.0/>
...
`/Applications/DigiByte-Qt` (on Mac) or `digibyted`/`digibyte-qt` (on
Linux).
...
`contrib/init/digibyted.service`, it has been changed to use
`/var/lib/digibyted` as the data directory instead of
`~digibyte/.digibyte`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/digibyted` will
exist has enough space (check using `df -h /var/lib/digibyted`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/digibyte/digibyte/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://digibyte.org/en/contact/) the DigiByte Core
...
The systemd init file (`contrib/init/digibyted.service`) has been changed
to use `/var/lib/digibyted` as the data directory instead of
`~digibyte/.digibyte`. This change makes DigiByte Core more consistent with
...
`datadir` option in `/etc/digibyte/digibyte.conf` will have no effect.
...
- A new short [document](https://github.com/digibyte/digibyte/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/digibyte/digibyte/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/digibyte/digibyte/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/digibyte/digibyte/blob/master/doc/digibyte-conf.md)
  about the `digibyte.conf` file describes how to use it to configure
  DigiByte Core.

- A new document introduces DigiByte Core's BIP174 [Partially-Signed
  DigiByte Transactions
  (PSBT)](https://github.com/digibyte/digibyte/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/digibyte/digibyte/blob/master/doc/descriptors.md)
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/digibyte/digibyte/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/digibyte/digibyte/blob/master/doc/descriptors.md).
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/digibyte/digibyte/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/digibyte/digibyte/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart digibyted with the `-deprecatedrpc=generate`
...
- A new `digibyte-wallet` tool is now distributed alongside DigiByte
...
- A sub-project of DigiByte Core now provides Hardware Wallet Interaction
  ...
  page](https://github.com/digibyte-core/HWI#readme) for details.
...
- #13926 [Tools] digibyte-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like digibyted (MarcoFalke)
...
- #14521 qt, docs: Fix `digibyte-qt -version` output formatting (hebasto)
...
- #14204 Move `interfaces/*` to `libdigibyte_server` (laanwj)
...
- #15154 Configure: digibyte-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/digibyte-wallet (Sjors)
...
- #15110 build\_msvc: Fix the build problem in `libdigibyte_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in digibyte-util-test.py (isghe)
...
- #12246 Bugfix: Only run digibyte-tx tests when digibyte-tx is enabled (luke-jr)
...
- #14186 digibyte-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_digibyte.conf` message on startup if conf file exists (leishman)
...
- #12255 Update digibyte.service to conform to init.md (dongcarl)
...
- #14497 Add doc/digibyte-conf.md (hebasto)
...
- #15354 Add missing `digibyte-wallet` tool manpages (MarcoFalke)
...
- #15765 correct digibyteconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support digibyte folder on OSX (JimmyMow)
