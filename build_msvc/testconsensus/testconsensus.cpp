...
// digibyte includes.
#include <..\src\script\digibyteconsensus.h>
...
int main()
{
    std::cout << "digibyteconsensus version: " << digibyteconsensus_version() << std::endl;
    ...
    auto op0Result = digibyteconsensus_verify_script_with_amount(pubKeyScript.data(), pubKeyScript.size(), amount, (const unsigned char*)&stream[0], stream.size(), 0, digibyteconsensus_SCRIPT_FLAGS_VERIFY_ALL, &err);
