...
Verify node behaviour and debug log when launching digibyted in these cases:

1. `digibyted` with no -asmap arg, using /16 prefix for IP bucketing

2. `digibyted -asmap=<absolute path>`, using the unit test skeleton asmap

3. `digibyted -asmap=<relative path>`, using the unit test skeleton asmap

4. `digibyted -asmap/-asmap=` with no file specified, using the default asmap

5. `digibyted -asmap` restart with an addrman containing new and tried entries

6. `digibyted -asmap` with no file specified and a missing default asmap file

7. `digibyted -asmap` with an empty (unparsable) default asmap file
...
class AsmapTest(DigiByteTestFramework):
    ...
    def test_without_asmap_arg(self):
        self.log.info('Test digibyted with no -asmap arg passed')
        self.stop_node(0)
        with self.node.assert_debug_log(['Using /16 prefix for IP bucketing']):
            self.start_node(0)

    def test_asmap_with_absolute_path(self):
        self.log.info('Test digibyted -asmap=<absolute path>')
        self.stop_node(0)
    ...
    def test_asmap_with_relative_path(self):
        self.log.info('Test digibyted -asmap=<relative path>')
        self.stop_node(0)
        name = 'ASN_map'
    ...
    def test_default_asmap(self):
        ...
        for arg in ['-asmap', '-asmap=']:
            self.log.info(f'Test digibyted {arg} (using default map file)')
            self.stop_node(0)
    ...
    def test_empty_asmap(self):
        self.log.info('Test digibyted -asmap with empty map file')
        self.stop_node(0)
