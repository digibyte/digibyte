...
#ifdef ENABLE_WALLET
    if(enableWallet) {
        connect(walletFrame, &WalletFrame::requestedSyncWarningInfo, this, &DigiByteGUI::showModalOverlay);
    }
#endif
...
void DigiByteGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, static_cast<void (QComboBox::*)(int)>(&QComboBox::currentIndexChanged), this, &DigiByteGUI::setCurrentWalletBySelectorIndex);
...
void DigiByteGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        if (optionsModel && trayIcon) {
            ...
            connect(optionsModel, &OptionsModel::hideTrayIconChanged, this, &DigiByteGUI::setTrayIconVisible);
...
void DigiByteGUI::setWalletController(WalletController* wallet_controller)
{
    ...
    connect(wallet_controller, &WalletController::walletAdded, this, &DigiByteGUI::addWallet);
...
void DigiByteGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
    ...
    labelWalletHDStatusIcon->setPixmap(platformStyle->SingleColorIcon(privkeyDisabled ? ":/icons/eye" : hdEnabled ? ":/icons/" + theme + "/hd_enabled" : ":/icons/" + theme + "/hd_disabled").pixmap(STATUSBAR_ICONSIZE,STATUSBAR_ICONSIZE));
