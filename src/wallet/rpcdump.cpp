...
void ImportAddress(CWallet*, const CDigiByteAddress& address, const std::string& strLabel);
void ImportScript(CWallet* const pwallet, const CScript& script, const std::string& strLabel, bool isRedeemScript)
{
    ...
    if (isRedeemScript) {
        ...
        ImportAddress(pwallet, CDigiByteAddress(CScriptID(script)), strLabel);
    } else {
...
void ImportAddress(CWallet* const pwallet, const CDigiByteAddress& address, const std::string& strLabel)
{
...
UniValue importpubkey(const JSONRPCRequest& request)
{
    ...
    ImportAddress(pwallet, CDigiByteAddress(pubKey.GetID()), strLabel);
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    for (std::vector<std::pair<int64_t, CKeyID> >::const_iterator it = vKeyBirth.begin(); it != vKeyBirth.end(); it++) {
        ...
        if (pwallet->GetKey(keyid, key)) {
            file << strprintf("%s %s ", CDigiByteSecret(key).ToString(), strTime);
...
UniValue ProcessImport(CWallet * const pwallet, const UniValue& data, const int64_t timestamp)
{
    try {
        ...
        if (!isScript) {
            address = CDigiByteAddress(output);
            if (!address.IsValid()) {
        ...
        if (isP2SH) {
            ...
            CDigiByteAddress redeemAddress = CDigiByteAddress(CScriptID(redeemScript));
            ...
            if (pubKeys.size() && keys.size() == 0) {
                ...
                CDigiByteAddress pubKeyAddress = CDigiByteAddress(pubKey.GetID());
                ...
                // Consistency check.
                if (isScript) {
                    CDigiByteAddress scriptAddress;
                    CTxDestination destination;

                    if (ExtractDestination(script, destination)) {
                        scriptAddress = CDigiByteAddress(destination);
