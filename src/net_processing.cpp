        ...
        while (it != pfrom->vRecvGetData.end() &&
            ...
            if (mi != mapRelay.end()) {
                ...
                if (!pfrom->fSupportsDandelion && !connman->isDandelionInbound(pfrom) && pfrom->setDandelionInventoryKnown.count(inv.hash)!=0) {
                    ...
                    if (txinfo.tx && txinfo.m_time <= mempool_req) {
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman* connman, BanMan* banman, const std::atomic<bool>& interruptMsgProc)
{
        ...
        for (CInv &inv : vInv)
        {
            ...
            if (inv.type == MSG_BLOCK) {
                ...
                if (fBlocksOnly) {
                    ...
                    pfrom->fDisconnect = true;
                    return true;
                } else if ((!fAlreadyHave && !fImporting && !fReindex && !::ChainstateActive().IsInitialBlockDownload() && connman->isDandelionInbound(pfrom)) || (inv.hash==dandelionServiceDiscoveryHash)) {
    ...
    if (strCommand == NetMsgType::TX) {
        ...
        TxValidationState state;
        TxValidationState dummyState; // Dummy state for Dandelion stempool
        ...
        if (!AlreadyHave(inv) &&
            ...
            AcceptToMemoryPool(stempool, dummyState, ptx, nullptr, false, 0);
            ...
            mempool.check(&::ChainstateActive().CoinsTip());
            // Changes to mempool should also be made to Dandelion stempool
            stempool.check(&::ChainstateActive().CoinsTip());
            RelayTransaction(tx.GetHash(), *connman);
