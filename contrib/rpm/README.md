...
The RPM spec file provided here is for DigiByte-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://digibyte.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[DigiByte git repository](https://github.com/digibyte/digibyte) but are not part of
...
Currently only `Source10` is used, for the example `digibyte.conf` file.
...
in the digibyte git repository. At present only an SVG version of the DigiByte
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the digibyte
...
needed when 0.12.1 is released, a proper fix is already in the DigiByte git
master branch.
...
idea to build DigiByte Core against a static version of that library compiled
...
The `digibyte.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `digibyte-server` package. At this point, most RPM based
...
The `digibyte-server` package should have SELinux support. How to properly do
...
DigiByte-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/digibyte/). He has been
packaging DigiByte for Fedora at least since 2012.
...
4. I renamed the `digibyte` package that contains the Qt GUI to `digibyte-core` as
that appears to be how the general population refers to it, in contrast to
`digibyte-xt` or `digibyte-classic`. I wanted to make sure the general population
