#!/bin/bash
# Copyright (c) 2013 The DigiByte Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete DigiByte database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no DigiByte datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old DigiByte datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected DigiByte 0.7 datadir."
    ;;
  3)
    echo "Detected DigiByte pre-0.8 datadir."
    ;;
  4)
    echo "Detected DigiByte 0.8 datadir."
    ;;
esac
