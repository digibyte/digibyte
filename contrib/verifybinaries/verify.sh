...
###   digibyte.org and digibyte.org and compares them.
...
if [ $? -ne 0 ]; then
   echo "digibyte.org failed to provide signature file, but digibyte.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "digibyte.org and digibyte.org signature files were not equal?"
